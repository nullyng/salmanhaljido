package com.salmanhaljido.demo.domain.sportsfacilities.service;
...
@Service
public class SportsFacilitiesServiceImpl implements SportsFacilitiesService {
    @Override
    public void getData() throws Exception {
        String dataPath = "src/main/resources/data/";
        URL url = new URL("https://www.localdata.go.kr/datafile/each/10_37_01_P.xlsx");

        File destination = new File(dataPath + "sportsfacilities.xlsx");
        FileUtils.copyURLToFile(url,destination);

        FileInputStream fileInputStream = new FileInputStream(dataPath + "sportsfacilities.xlsx");
        XSSFWorkbook workbook = new XSSFWorkbook(fileInputStream);

        FileOutputStream fileOutputStream = new FileOutputStream(dataPath + "sportsfacilities.data");
        ...
        SparkSession session = SparkSession.builder()
                .master("local")
                .appName("sportsfacilities")
                .config("spark.mongodb.write.connection.uri", "mongodb://admin:salmand110@j7d110.p.ssafy.io/openapi.sportsfacilities?authSource=admin")
                .getOrCreate();

        Dataset<Row> df = session.read().text(dataPath + "sportsfacilities.data");
        ...
        File writeFile = new File(dataPath + "sportsfacilities_result.json");
