...
public class ShelterServiceImpl implements ShelterService {
    ...
    public void getData() throws Exception {
        ...
        URL url = new URL("https://www.localdata.go.kr/datafile/each/11_46_02_P.xlsx");

        File destination = new File(dataPath + "shelter.xlsx");
        FileUtils.copyURLToFile(url,destination);

        FileInputStream fileInputStream = new FileInputStream(dataPath + "shelter.xlsx");
        XSSFWorkbook workbook = new XSSFWorkbook(fileInputStream);

        FileOutputStream fileOutputStream = new FileOutputStream(dataPath + "shelter.data");
        ...
        SparkSession session = SparkSession.builder()
                .master("local")
                .appName("shelter")
                .config("spark.mongodb.write.connection.uri", "mongodb://admin:salmand110@j7d110.p.ssafy.io/openapi.shelter?authSource=admin")
                .getOrCreate();

        Dataset<Row> df = session.read().text(dataPath + "shelter.data");
        ...
        File writeFile = new File(dataPath + "shelter_result.json");
