...
@Service
public class AnimalSalonServiceImpl implements AnimalSalonService {

    @Override
    public void getAnimalSalonData() throws IOException {
        String dataPath = "src/resources/data/";

        URL url = new URL("https://www.localdata.go.kr/datafile/each/02_03_11_P.xlsx");

        File destination = new File(dataPath + "animalsalon.xlsx");
        FileUtils.copyURLToFile(url,destination);

        FileInputStream fileInputStream = new FileInputStream(dataPath + "animalsalon.xlsx");
        XSSFWorkbook workbook = new XSSFWorkbook(fileInputStream);

        FileOutputStream fileOutputStream = new FileOutputStream(dataPath + "as.data");
        ...
        SparkSession session = SparkSession.builder()
                .master("local")
                .appName("as")
                .config("spark.mongodb.write.connection.uri", "mongodb://127.0.0.1/openapi.as")
                .getOrCreate();

        Dataset<Row> df = session.read().text(dataPath + "as.data");
        ...
        File writeFile = new File(dataPath + "as_result.json");
        ...
        Dataset<Row> dff = session.read().format("json").load(dataPath + "as_result.json");
